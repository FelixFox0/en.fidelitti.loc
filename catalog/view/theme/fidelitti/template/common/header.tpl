<?  
    $_SERVER['REQUEST_URI'] = substr($_SERVER['REQUEST_URI'], 6, 200);
    if ($_SERVER['REQUEST_URI'] == '/deliverys'){
        header( 'Location: /deliverys_en', true, 301 );
        exit;
    }
    if ($_SERVER['REQUEST_URI'] == '/payment'){
        header( 'Location: /payment_en', true, 301 );
        exit;
    }
