...
//BOF Product Series	
			...
			$data['sorts'][] = array(
				...
				'href'  => $this->url->link($route, $path
			 . '&sort=p2c.sort_order&order=ASC' . $url) //! FOR CUSTOM SORT
			);
/*
			$data['sorts'][] = array(
...
//BOF Product Series	
			...
			$pagination->url = $this->url->link($route, $path
			 . $url . '&page={page}');

			$data['totalproducts'] = $product_total; $data['default_limit'] =  $this->config->get($this->config->get('config_theme') . '_product_limit'); $data['limit']=$limit;
            $data['page']=$page;
